...
<table width="782" border="1" bordercolor="#99CCFF" cellpadding="0" cellspacing="0" style="border-collapse:collapse">
  <!--DWLayoutTable-->
  <tr>
    <td width="188" height="122" valign="top"><table width="98%" border="0" bordercolor="#99CCFF" cellpadding="0" cellspacing="0" style="border-collapse:collapse">
          <!--DWLayoutTable-->
		 <tr>
	   ...
	   <tr>
		  ...
		  <%
			...
			float price=0;
		  	String psql="select top 5 p_id,p_name,p_price,p_image from product where p_type='"+t_type+"' order by p_price asc";
	...
	<font size="1">原价：<%=price+price*0.3%> </font> </td>
	    ...
	    <tr>
	     <td height="7" align="center" valign="top"><span class="STYLE1">现价</span>：<font size="2" color="#FF0000">￥<%= price %></font></td>
	     </tr>
	   <tr>
...
Connection conn=null;
	...
	try
	{
		...
		String sql="select p_id,p_name,p_price,p_image,p_description from product where p_type='"+t_type+"'";
...
%>
  ...
  <% 
   ...
   %>
    ...
    <td width="100" align="center"><p><span class="STYLE5">============</span></p><p class="STYLE1"><%= p_name %></p><p><font size="2" color="#FF0000">￥<%= p_price %></font></p>   
	<p><a href="#" onClick="window.open('view_product.jsp?p_id=<%= p_id %>','help','scrollbars=yes,resizable=yes,width=600,height=520')"><input name="see" type="button" class="STYLE1" value="详情"></a><%if (c_name!=null && c_name!=""){%><input name="see" type="button" class="btn_grey" onClick="window.location.href='cart_add.jsp?p_id=<%=p_id%>'" value="购买"><% } %></p>   </td>
    <% 
			}
